...
sub simple_guess {
	...
	$languageName = $got->[0]{name};
	if($isoCode = $self->{models2Code}{$languageName}) {
		$languageName = $isoCode;
#print "$languageName: $isoCode\n";
	}	
#print "$languageName\n";
	return $languageName;	
}
...
sub load_models {
	...
	while ( my $f = readdir $dh ) {
		...
		$models{$name} = \%model;
#print "$f: $name: $isoCode\n";	
		$models2Code{$name} = $isoCode;
	}
	$self->{models} = \%models;
	$self->{models2Code} = \%models2Code;
}
